...
#PRIMERSn="$PRIMERPATH"/"nimagen.fasta"; #phased out
...
rm "$Filein"*.sam.gz "$Filein"*.bam "$Filein"*.bai "$Filein"*.txt "$Filein"*.fa "$Filein"*.vcf;
		...
		reformat.sh in="$workdir"/RAWREADS/"$SAMPLE"_R#.fastq.gz out="$Filein".fq.gz ow=t > "$LOG1" 2>&1;
...
#		bbduk.sh in="$Filein"_viral.fq.gz out="$Filein"_recal.fq.gz recalibrate -Xmx1g ow > "$LOG3" 2>&1;
		...
		bbmerge.sh in="$Filein"_viral.fq.gz outa="$Filein"_adapters.fa ow reads=1m > "$LOG4" 2>&1;

		# 5
		##  Remove duplicates by sequence similarity.
		##  This is more memory-efficient than dedupebymapping.
		clumpify.sh in="$Filein"_viral.fq.gz out="$Filein"_clumped.fq.gz zl=9 dedupe s=2 passes=4 -Xmx31g > "$LOG5" 2>&1;
		...
		bbduk.sh in="$Filein"_trimmed.fq.gz out="$Filein"_trimmed2.fq.gz ref="$PRIMERS" ktrim=l restrictleft=30 k=22 hdist=3 qhdist=1 rcomp=f mm=f > "$LOG7" 2>&1;
		...
		applyvariants.sh in="$REF" out="$Filein"_genome.fa vcf="$Filein"_vars.vcf basecov="$Filein"_basecov_border5.txt ow mindepth="$MINCOV" > "$LOG15" 2>&1;
