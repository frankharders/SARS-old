...
cp "$PWD"/02_polished/*/*_trimclip.sam.gz  "$PWD"/sumAnalysis;
...
callvariants.sh "$PWD"/sumAnalysis/*_trimclip.sam.gz ref="$REF" multisample out=allVars.vcf ow -Xmx4g usebias=f strandedcov minstrandratio=0 maf=0.6 minreads="$MINCOV" mincov="$MINCOV" minedistmax=30 minedist=16 flagnearby > all.samples.variants.log 2>&1;
##  Make a summary of coverage at varous depth cutoffs for all libraries.
summarizecoverage.sh "$PWD"/sumAnalysis/*basecov_border5.txt out=coverageSummary.txt > "$PWD"/all.samples.summary.log 2>&1;
mkdir -p output;
cp *.sh output;
...
SOFTWARE="$PWD"/output/software.versions.used.txt

echo "Info conda version" > "$SOFTWARE";
conda info >> "$SOFTWARE";

echo "list of packages installed in environment" >> "$SOFTWARE";
conda list >> "$SOFTWARE";

echo "Info Samtools version used:" >> "$SOFTWARE";
